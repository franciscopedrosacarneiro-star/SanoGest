...
if (($documento->tipo_associacao ?? '') === 'Equipamento') {
    $id_entidade_atual = $documento->id_equipamento ?? '';
}

if (($documento->tipo_associacao ?? '') === 'Fornecedor') {
    $id_entidade_atual = $documento->id_fornecedor ?? '';
}

if (($documento->tipo_associacao ?? '') === 'Garantia' || ($documento->tipo_associacao ?? '') === 'Contrato') {
    $id_entidade_atual = $documento->id_garantia ?? '';
}

$valores = (object) array_merge((array) $documento, [
    'id_entidade_associada' => $id_entidade_atual
]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validar_dados_documento($dados)) {
        ...
        $valores = (object) array_merge((array) $documento, $dados);
    } else {
        try {
            $resultado = atualizar_documento($pdo, $id_documento, $dados);
...
<head>
    ...
    <title>SanoGest | Editar Documento</title>
    ...
    <main class="p-5 fundo-dashboard conteudo-principal">
        <div class="bg-white bg-opacity-75 p-5 rounded shadow-sm">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="consultar.php?id_documento=<?= htmlspecialchars($id_documento) ?>" class="btn btn-outline-secondary">
            ...
            <form action="editar.php?id_documento=<?= htmlspecialchars($id_documento) ?>" method="POST" novalidate>
                                ...
                                <select id="id_entidade_associada" name="id_entidade_associada" class="form-select" required>
                                    ...
                                    <?php foreach ($equipamentos as $equipamento): ?>
                                        <option 
                                            ...
                                            <?= (($valores->tipo_associacao ?? '') === 'Equipamento' && (string) ($valores->id_entidade_associada ?? '') === (string) $equipamento->id_equipamento) ? 'selected' : '' ?>
                                        >
                                    ...
                                    <?php foreach ($fornecedores as $fornecedor): ?>
                                        <option 
                                            ...
                                            <?= (($valores->tipo_associacao ?? '') === 'Fornecedor' && (string) ($valores->id_entidade_associada ?? '') === (string) $fornecedor->id_fornecedor) ? 'selected' : '' ?>
                                        >
                                    ...
                                    <?php foreach ($garantias as $garantia): ?>
                                        <option 
                                            ...
                                            data-tipo="Garantia"
                                            <?= (($valores->tipo_associacao ?? '') === 'Garantia' && (string) ($valores->id_entidade_associada ?? '') === (string) $garantia->id_garantia) ? 'selected' : '' ?>
                                        >
                                        ...
                                        <option 
                                            value="<?= htmlspecialchars($garantia->id_garantia) ?>"
                                            data-tipo="Contrato"
                                            <?= (($valores->tipo_associacao ?? '') === 'Contrato' && (string) ($valores->id_entidade_associada ?? '') === (string) $garantia->id_garantia) ? 'selected' : '' ?>
                                        >
                                            Contrato |
                ...
                <div class="d-flex justify-content-between align-items-center mt-4">
                    ...
                    <div>
                        <a href="consultar.php?id_documento=<?= htmlspecialchars($id_documento) ?>" class="btn btn-secondary me-2">
                            Cancelar
                        </a>
